...
def fit_VGP(data, kern, test_data, max_iter=200):

    f1scores = []
    elbos = []

    def train_callback(*args, **kargs):
        # pred_train = (invlink(m.predict_f(data[0])[0].numpy().squeeze()) > 0.5).astype(
        #     int
        # )
        # f1scores.append(f1_score(data[1], pred_train))
        elbos.append(m.elbo().numpy())

    m = gpflow.models.VGP(data, kernel=kern, likelihood=gpflow.likelihoods.Bernoulli())
...
def fit_GPMC(data, kern, test_data):
    ...
    m = gpflow.models.GPMC(data, kernel=kern, likelihood=gpflow.likelihoods.Bernoulli())

    opt = gpflow.optimizers.Scipy()
    opt.minimize(
        m.training_loss, variables=m.trainable_variables, options={"maxiter": 500}
    )
    ...
    for i in range(burn, n_samples, thin):
        for var, var_samples in zip(m.trainable_variables, samples):
            var.assign(var_samples[i])
        pred_train = (
            np.squeeze(invlink(m.predict_f_samples(X_train, 10))).mean(0) > 0.5
        ).astype(int)
        f1train.append(f1_score(y_train, pred_train))

    pred_test = (
        invlink(m.predict_f_samples(X_test, 20).numpy().squeeze().T).mean(1) > 0.5
    ).astype(int)
