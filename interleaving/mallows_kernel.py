...
def approx_gram_mat_topk(
    my_kernel, mykernargs, partial_rankings, n_mc_samples, n_deg, rank_only=False
):
    ...
    if not rank_only:
        ...
        for ell in range(n_rankings):
            for kk in range(n_rankings):
                K_aux = compute_kernel_matrix(
                    my_kernel, mykernargs, complete_rankings[ell], complete_rankings[kk]
                )
    ...
    return (
        (complete_rankings_flat, K_mat_mc) if not rank_only else complete_rankings_flat
    )


class Mallows(Kern):
    def __init__(
        self, input_dim, variance=1.0, scale=1.0, active_dims=None, name="mallows"
    ):
    ...
    def K(self, X, X2=None):
        return self.variance * pairwise_distances(
            X, X2, lambda u, v: self.kernel_mallows(u, v)
        )
...
class Mallows_PRMC(Kern):
    ...
    def kernel_pr(self, sigmas, sigmaprimes):
        ...
        assert (
            type(n_fullrankings) is int
        ), "dimension of array is {} and ndeg is {}".format(np.shape(sigmas)[1], ndeg)
        sigmas_unf = np.reshape(sigmas, ((n_fullrankings, ndeg)))
        n_fullrankings_prime = len(sigmaprimes) // ndeg
        assert (
            type(n_fullrankings_prime) is int
        ), "dimension of array is {} and ndeg is {}".format(np.shape(sigmas)[1], ndeg)
        ...
        for ii in range(n_sigmas):
            for jj in range(n_sigmaprimes):
                ...
                ker_val += self.kernel_mallows(
                    sigmas_unf[ii, :], sigmaprimes_unf[jj, :]
                )
    ...
    def K(self, X, X2=None):
        return self.variance * pairwise_distances(
            X, X2, lambda u, v: self.kernel_pr(u, v)
        )
...
class MallowsGPy(GPy.kern.RBF):
    def __init__(
        ...
        super(MallowsGPy, self).__init__(
            input_dim, variance, lengthscale, ARD, active_dims, name, useGPU=useGPU
        )
    ...
    def _unscaled_dist(self, X, X2=None):
        ...
        if X2 is None:
            ...
            view(r2)[
                :,
            ] = 0.0  # force diagnoal to be zero: sometime numerically a little negative
...
def get_phi_topk(X, n_mc_samples, N_objs, train_ind, test_ind):
    X_completerankings = approx_gram_mat_topk(
        kernel_mallows, 1, X, n_mc_samples, N_objs, True
    ).astype(int)
    X_completerankings = X_completerankings.reshape(len(X) * n_mc_samples, -1)
    X_, _ = get_phi_mallows(
        X_completerankings, N_objs, np.arange(len(X_completerankings)), np.arange(3)
    )
