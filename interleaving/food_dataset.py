...
def synthetic_rank(pref, df_food, rng, sigma):
    importance = (5 * np.array(pref) + 1.0) ** -1
...
def food_dataset(
    ...
    df_food = (
        pd.DataFrame(
            ...
            index=["sweet", "savoury", "juicy"],
        )
        / 10
    )

    groups_ind = [[0, 1, 2], [3, 4, 5], [6, 7]]  # Sweet  # Meat  # Carbs
    ...
    pref = [0, 1, 2]
    for i in range(N // 2):
        df_ranks.iloc[i] = synthetic_rank(pref, df_food, rng, sigma)
    ...
    if rank_type == "full":
        ...
        for i in range(len(X)):
            ...
            for s in X[i]:
                cur.append(set([s]))
            if len(cur) != N_objs:
                cur.append(set(range(N_objs)) - set(X[i]))
            X_ours[i] = cur
        ...
        X_ours = np.empty(len(X), np.object)
        for i in range(len(X)):
            inter_length = N_objs - rng.randint(min_removed, max_removed)
            X_ours[i] = list(
                np.array(X[i])[
                    sorted(np.random.choice(range(N_objs), inter_length, False))
                ]
            )
        # X = np.vstack(X)
        X_ours = X_ours.tolist()
