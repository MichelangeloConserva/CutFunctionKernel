...
try:
    ...
    raise ImportError(
        "Make sure that rpy2 is installed and that kernrnak packgage is installed in R"
    )
...
class KendallTOPk(Kern):
    ...
    def __init__(
        self, input_dim, N_objs, variance=1.0, active_dims=None, name="kendall"
    ):
    ...
    def K(self, X, X2=None):
        return self.variance * pairwise_distances(
            X, X2, lambda u, v: self.kernel_kendall(u, v)
        )
...
class KendallPartialflow(gpflow.kernels.base.Kernel):
    def __init__(self, K, N_objs, variance=1.0):
        ...
        self.variance = gpflow.base.Parameter(
    ...
    def K(self, X, X2):
        ...
        if X2 is None:
            ...
            X2 = tf.cast(tf.squeeze(X2), tf.int32)
